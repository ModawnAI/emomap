class jsCogtask extends Object {
    ...
    set_pretest_session_loop(pretest_session_loop){
        if(this.use_pretest_session==false) return;
    ...
    add_prolog_trials(prolog_trials)
    {
        ...
        if (this.prolog_trials!==null) {
            if(!Array.isArray(this.prolog_trials)) this.prolog_trials=[this.prolog_trials];
    ...
    compile(trialall=true,envmode=false,pretestmode=this.use_pretest_session) {
        if(this.blankmode) {
    ...
    evaluate_performance(targetcode,nontargetcode)
    {
        ...
        let result={};let sdata,cdata,rdata;
        ...
        rdata=trial_sequence.filterCustom(function(trial){return trial.button_pressed==undefined ? false:true;});
        if(result.total_count>0){
            result.total_rt_mean = rdata.select('rt').mean();
            result.total_rt_sd = rdata.select('rt').sd();
        ...
        rdata=sdata.filterCustom(function(trial){return trial.button_pressed==undefined ? false:true;});
        if(result.target_count>0){
            result.target_rt_mean = rdata.select('rt').mean();
            result.target_rt_sd = rdata.select('rt').sd();
        } else {
        ...
        rdata=cdata.filterCustom(function(trial){return trial.button_pressed==undefined ? false:true;});
        if(result.target_correct_count>0){
            result.target_correct_rt_mean = rdata.select('rt').mean();
            result.target_correct_rt_sd = rdata.select('rt').sd();
        } else {
        ...
        if(result.target_incorrect_count>0){
            result.target_incorrect_rt_mean = rdata.select('rt').mean();
            result.target_incorrect_rt_sd = rdata.select('rt').sd();
        } else {
        ...
        if(result.nontarget_count>0){
            result.nontarget_rt_mean = rdata.select('rt').mean();
            result.nontarget_rt_sd = rdata.select('rt').sd();
        } else {
        ...
        result.nontarget_correct_unresponded_count=cdata.filter({button_pressed:undefined,}).count();
        rdata=cdata.filterCustom(function(trial){return trial.button_pressed==undefined ? false:true;});
        if(result.nontarget_correct_count>0){
            result.nontarget_correct_rt_mean = rdata.select('rt').mean();
            result.nontarget_correct_rt_sd = rdata.select('rt').sd();
        } else {
        ...
        result.nontarget_incorrect_unresponded_count=cdata.filter({button_pressed:undefined,}).count();
        rdata=cdata.filterCustom(function(trial){return trial.button_pressed==undefined ? false:true;});
        if(result.nontarget_incorrect_count>0){
            result.nontarget_incorrect_rt_mean = rdata.select('rt').mean();
            result.nontarget_incorrect_rt_sd = rdata.select('rt').sd();
        } else {
    ...
    get_debrief_type(mode,targetcodes=null){
        ...
        if(mode==='congruent'){     
            ...
            let tscore=this.base_score+this.performance.total_score;
            html= "%GUIDE%"+
            "결과는 다음과 같습니다.<br>총 시행자극수: <strong>" + this.performance.total_count + "회 총점수: " +tscore+ " </strong><br>"+
    ...
    run(){
        ...
        jsPsych.init({
            ...
            max_load_time:this.max_load_time,
